...
*/
		...
		return {
			...
			restoreGuiSettings: function(settings) {
				$.each( settings, function( key, value ) {
					if (key.indexOf('-panel-') !== -1) {
						var $panel = $("[data-settings='" + key + "']");
						if ($panel.length > 0) {
							$.each( value, function( key, value ) {
								switch (key) {
									case 'fullscreen': 
										if ($panel.hasClass('panel-fullscreened') !== (value === 'true')) {
											panelFullscreen($panel);
										}
										break;
									case 'collapse': 
										if ($panel.hasClass('panel-toggled') !== (value === 'true')) {
											panelCollapse($panel);
										}
										break;
									case 'remove': 										
										if ($panel.hasClass('panel-removed') !== (value === 'true')) {
											panelRemove($panel);
										}
										break; 
								/*	case 'refresh':		break; */
			...
			setPanelsInteractions: function(userId) {
				$('.panel-fullscreen').on('click', function () {
					var $panel = $(this).parents('.panel');
					panelFullscreen($panel, function(state) {
						var settings = {};
						settings[$panel.data('settings')] = {'fullscreen': state};
						$.post( '/api/users/setSettings', {'userId': userId, 'settings': settings} 
						).done( function() {})
						.fail( function() {});	
					});
				...
				$('.panel-collapse').on('click', function () {
					var $panel = $(this).parents('.panel');
					panelCollapse($panel, function(state) {
						var settings = {};
						settings[$panel.data('settings')] = {'collapse': state};
						$.post( '/api/users/setSettings', {'userId': userId, 'settings': settings} 
						).done( function() {})
						.fail( function() {});	
					});
				...
				$('.panel-remove').on('click', function () {
					var $panel = $(this).parents('.panel');
					panelRemove($panel, function(state) {
						var settings = {};
						settings[$panel.data('settings')] = {'remove': state};
						$.post( '/api/users/setSettings', {'userId': userId, 'settings': settings} 
						).done( function() {})
						.fail( function() {});	
					});
			...
			/*	J'ai un cas d'usage particulier sur dashboard avec les news.
				verifier ça avant de décommenter dette fonction
			...
			*/
			...
			setTabsInteractions: function(userId) {
				$('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
					//e.target // newly activated tab
					//e.relatedTarget // previous active tab
					var $a = $(e.target);
					var $tab = $a.parents('.panel');
					var id = $a.attr('href');
					var settings = {};
					settings[$tab.data('settings')] = id;
					$.post( '/api/users/setSettings', {'userId': userId, 'settings': settings} 
					).done( function() {})
					.fail( function() {});	
				})
			...
			saveTableSettings: function(userId, tableName, data) {
				var settings = {};
				settings[tableName] = data;
				$.post( '/api/users/setSettings', {'userId': userId, 'settings': settings} 
				).done( function() {})
				.fail( function() {});	
			},
			...
			getTableSettings: function(userId, tableName, cb) {
				$.post( '/api/users/getSettings', {'userId': userId} 
				).done( function(settings) {
					var err = new Error('Could not find user settings');
					err.status = 404;
					if (settings) {
						var data;
						$.each( settings, function( key, value ) {
			...
			setNavigationInteractions: function() {
				xNavigation();
				onLoad();
