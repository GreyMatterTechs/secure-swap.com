...
module.exports = function (Admin) {
	...
	*/
	...
	Admin.sendEmailLicence = function(tokenId, userId, robotName, strategyName, fn) {
		fn = fn || utils.createPromiseCallback();
		var err;
		if (!tokenId) {
			err = new Error(g.f('{{accessTokens}} is required to send Email Licence'));
			err.status = 401;
			process.nextTick(fn, err);
			return fn.promise;
		}		
		Admin.findById(userId, function(err, user) {
			if (err) {
				...
				var options			= makeOptions();
				options.template	= path.resolve(__dirname, '../../server/views/signLicence_email.ejs');
				options.to			= user.email;
				options.subject		= '[' + config.appName + '] Signature des Licences';
				options.firstname	= user.prenom;
				options.sName		= strategyName;
				options.rName		= robotName;				
				//options.verifyHref	= options.protocol + '://' + options.host + displayPort + '/reset-password' + '?access_token=' + info.accessToken.id;
				...
				err = new Error(g.f('Can\'t found USer with id:%s', userId));
				err.statusCode = 404;
				err.code = 'SENDEMAILLICENCE_FAILED';
				return fn(err, null);
			}
		});		
		return fn.promise;
	};
    Admin.remoteMethod( 'sendEmailLicence', {
		accepts: [
			{arg: 'access_token', type: 'string', http: function(ctx) {
				var req = ctx && ctx.req;
				var accessToken = req && req.accessToken;
			...
			{arg: 'robotName', type: 'string', required: true}, 
			{arg: 'strategyName', type: 'string', required: true}
		],
		returns: {
			arg: 'sent', type: 'boolean', root: true
		},
		http: {verb: 'POST'}
	});
